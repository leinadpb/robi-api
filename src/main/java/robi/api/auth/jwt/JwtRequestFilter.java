...
import robi.api.common.MutableHttpServletRequest;
...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        String mathUrl = request.getServletPath();
        String method = request.getMethod();
        if (method.equalsIgnoreCase("options")) {
            return true;
        }
        // Login method public
        return mathUrl.contains("auth/login");
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (requestTokenHeader != null && requestTokenHeader.startsWith("Bearer ")) {
            ...
            try {
                ...
                LOGGER.error("Username couldn't be extracted from provided Bearer token.");
            }
        } else {
        ...
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            try {
                ...
                if (jwtTokenUtil.validateToken(jwtToken, userDetails)) {
                    ...
                    // Set request headers
                    mutableRequest.putHeader("user_id", String.valueOf(userId));
                    mutableRequest.putHeader("user_name", userDetails.getUser().getUsername());
                }
            } catch (Exception e) {
